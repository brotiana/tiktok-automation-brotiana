...
class InputMethodMixIn(AbstractShell):
    ...
    def set_input_ime(self, enable: bool = True):
        ...
        if self.current_ime() == self.__ime_id:
            return
        # prepare ime
        ime_list = self.__get_ime_list()
        if self.__ime_id not in ime_list:
            self._setup_ime()
        # Sur Realme/ColorOS, ime list peut retourner vide à cause de SecurityException
        # On ignore le assert et on continue
    ...
    def _setup_ime(self):
        ...
        # Vérifier si le package est déjà installé pour éviter une réinstallation inutile
        pkg_name = self.__ime_id.split('/')[0]
        result = self.shell(['pm', 'list', 'packages', pkg_name])
        already_installed = pkg_name in result.output
        ...
        if not already_installed:
            with with_package_resource("assets/app-uiautomator.apk") as ime_apk_path:
                try:
                    ...
                    self.adb_device.uninstall(pkg_name)
                    self.adb_device.install(str(ime_apk_path), nolaunch=True, uninstall=True)
        else:
            logger.debug("AdbKeyboard already installed, skipping install")
        ...
        for _ in range(10):
            if self.__ime_id in self.__get_ime_list():
                return
            time.sleep(.3)
        # Realme/ColorOS bloque ime list via SecurityException
        # On ignore l'erreur et on continue quand même
        logger.warning("AdbKeyboard ime list check failed (Realme restriction), continuing anyway...")
